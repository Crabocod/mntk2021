$('textarea#prof_top_text').ready(function() {
    var urlSegments = getUrlSegments(document.location.pathname);

    setEditor(document.getElementById('prof_top_text'), {
...
$(document).ready(function () {
    ...
    $('body').on('click', '.toggle-list a.unselectable:not([disabled])', function (e) {
        e.preventDefault();
    ...
    $('body').on('change', '.row-file input', function (e) {
        e.preventDefault();
        if($(this).val() == '')
            return false;
        var fileName = $(this).val().split('/').pop().split('\\').pop();
        $(this).siblings('span').text(fileName);
        $(this).siblings('img.file__delete').css('display', 'inherit');
        $(this).closest('.thumbnail_div').before(newRow);
    });

    $('body').on('click', '.row-file .file__delete', function (e) {
        e.preventDefault();
...
function saveProfQuizText(form) {
...
function update(row) {
    ...
    formData.append('id', row.find('input[name=questions_id]').val());
    ...
    var formDataFile = new FormData(row.find('form')[2]);
