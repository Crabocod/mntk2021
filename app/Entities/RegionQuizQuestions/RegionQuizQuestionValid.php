...
class RegionQuizQuestionValid extends Entity
{
    public static function add(array $data = [])
    {
        $valid = Services::validation();
        $valid->setRule('region_quiz_question_id', 'ID вопроса', 'required|is_natural_no_zero');
    ...
    public static function save(array $data, Request $request = null)
    {
        $valid = Services::validation();
        if(isset($data['id']))
            $valid->setRule('id', 'ID вопроса', 'required|is_natural_no_zero|is_not_unique[region_quiz_questions.id]');
    ...
    {
        $valid = Services::validation();
        $valid->setRule('id', 'ID викторины', 'required|is_natural_no_zero|is_not_unique[region_quiz_questions.id]');
